...
def RBGS(pressure, div, iterations):
    ...
    for it in range(iterations):
        ...
        # red
        i, j = i0, 2*j0 + (i0 % 2)
        new_pressure[i,j] = (pressure[i-1, j] + pressure[i+1, j] + pressure[i, j-1] + pressure[i, j+1] - div[i, j]) / 4.0

        # black
        i, j = i0, 2*j0 + ((i0+1) % 2)
        new_pressure[i,j] = (new_pressure[i-1, j] + new_pressure[i+1, j] + new_pressure[i, j-1] + new_pressure[i, j+1] - div[i, j]) / 4.0
...
def VCycle(pressure, div):
    ...
    res = Restrict(res)
    pressure0 = RBGS(tf.zeros(res.shape), 4.0*res, 5)
