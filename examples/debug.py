...
def mandelbrot():
    ...
    canvas[i, j, 0] = (smoothstep(0.0, 1.0, 2.5 * (rx/norm) ** 0.9) * 0.2 + prev_frame[i, j, 0] * 0.8)
    canvas[i, j, 1] = (smoothstep(0.0, 1.0, 2.5 * (ry/norm) ** 0.65) * 0.2 + prev_frame[i, j, 1] * 0.8)
    canvas[i, j, 2] = (smoothstep(0.0, 1.0, 2.5 * (rz/norm) ** 0.5) * 0.2 + prev_frame[i, j, 2] * 0.8)
...
tf.show_window(800, 600, "Mandelbrot")
