...
class PSI(tf.Module):
    def __init__(self, atom_n = 1, electron_n = 2):
        ...
        self.orbi_layer0_bias = tf.Parameter([4], tf.float32, random_scale = 0.0)
        self.orbi_layer1 = tf.Parameter([4*self.atom_n, 8], tf.float32)
        self.orbi_layer2 = tf.Parameter([8, 1], tf.float32)
    ...
    def psi_new(self, electrons):
        ...
        envelope = tf.unsqueeze(tf.exp(-0.25 * r))
        in1 = out0 * envelope
        in1 = tf.reshape(in1, [electrons.shape[0], self.electron_n, self.atom_n * 4])

        out1 = in1 @ self.orbi_layer1

        orbisum = tf.sum(tf.squeeze(out1 @ self.orbi_layer2))

        b, i = tf.indices([electrons.shape[0], 3])
        r12 = tf.norm(electrons[b, 0, i] - electrons[b, 1, i])
        jastrow = lognum(self.gamma*r12)

        return aslog(orbisum) * jastrow
