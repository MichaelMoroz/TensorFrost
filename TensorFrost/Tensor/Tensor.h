...
class Shape {
 public:
  ...
  explicit Shape(std::vector<Dimension> dimensions) { this->dimensions = std::move(dimensions); }
...
class Tensor {
 private:
  ...
  static shared_ptr<Tensor> Op(const std::string& op, const Args*... args) {
    ...
    for (int i = 0; i < tensors.size(); i++) {
      arguments.emplace_back(Argument::Type::Input, const_cast<Tensor*>(tensors[i]), i);
    }
 ...
 public:
  ...
  Tensor(float* data, std::vector<int> sizes)
      : Tensor(std::move(sizes), vector<Argument>(), "const_memory", DataType::f32) {
  ...
  [[nodiscard]] static double get(const std::vector<int>&  /*indices*/) { return 0.0; }
